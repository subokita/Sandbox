...
string filename = "/Users/saburookita/Desktop/sample2.jpg";
...
void testImage() {
    ...
    Mat edge;
    cvtColor( frame, edge, CV_BGR2GRAY );
    Canny( edge, edge, 30.0, 90.0 );
    
    /* Vote for the accumulation matrix */
    detector.vote( edge, 25, 250 );
    
    /* Draw the symmetrical line */
    vector<pair<Point, Point>> result = detector.getResult( 4 );
    for( auto point_pair: result )
